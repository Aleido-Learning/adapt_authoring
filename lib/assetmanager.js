    ...
    return {
      ...
      moduleLoaded : function (modloaded) {
        ...
        if(modloaded === WAITFOR){
          ...
          rest.post('/asset/replace', instance.replaceAsset.bind(instance));
          // rest.post('/replace', instance.replaceAsset.bind(instance));
          ...
          rest.put('/asset/delete/:id', instance.deleteAsset.bind(instance));
          // rest.put('/asset/replace/:id', instance.replaceAsset.bind(instance));
          ...
          rest.get('/asset/uses/:id', instance.assetUses.bind(instance));
...
/**
   * processes an uploaded asset to replace an existing asset
   *
   * @param {object} res
   * @param {object} req
   * @param {callback} next
...
replaceAsset: function (req, res, next) {
  const _respondError = (message, code = 500) => {
    res.status(code).json({ success: false, message: message })
  };
  var form = new IncomingForm();
...
/**
   * gets uses of an asset from master tenant
   *
   * @param {object} req
   * @param {object} res
   * @param {callback} next
...
assetUses: async function (req, res, next) {
  ...
  try {
    const db = await new Promise((resolve, reject) => {
      database.getDatabase((err, db) => err ? reject(err) : resolve(db));
    });

    const retrieve = util.promisify(db.retrieve.bind(db));

    const assetRecs = await retrieve('asset', {_id: req.params.id});
    var filename = assetRecs[0].filename;

    const courseRecs = await retrieve('course', {});
    if (!courseRecs.length) return next(new Error("No courses found"));

    for (const course of courseRecs) {
      tempCourseId = course._id.toString();
      const componentRecs = await retrieve('component', {});
      for (const component of componentRecs) {
        if(component._courseId.toString() === course._id.toString()){
          var jsonString = JSON.stringify(component);
          if (jsonString.includes(filename)) { 
              assetUsed++;
              let courseTitleStr = course.displayTitle !== undefined ? course.displayTitle : course.title;
              let courseIdStr = course._uniqueId !== undefined ? course._uniqueId : course._id;
              let titleStr = component.displayTitle !== undefined ? component.displayTitle : component.title;
              let idStr = component._uniqueId !== undefined ? component._uniqueId : component._id;
              if(assetUsed > 1) listComponents += ',';
              listComponents += '{ "courseId" : "' + courseIdStr + '", "id" : "' + idStr + '", "title" : "' + titleStr + '"}';
              // console.log(assetUsed);
              if(tempCourseId !== lastCourseId) {
                coursesUsed++;
                // console.log(coursesUsed);
                lastCourseId = tempCourseId;
                if(coursesUsed > 1) listCourses += ',';
                listCourses += '{ "id" : "' + courseIdStr + '", "title" : "' + courseTitleStr + '"}';
              }
            }
        }
      }
    ...
    res.statusCode = 200;
    console.log('res', assetUsed, coursesUsed);
    listCourses += ']';
    listComponents += ']';
    // "uses" :' + assetUsed + ', "courseUses" :' + coursesUsed + ', 
    let json = '{ "courses" : ' + listCourses + ', "components" : ' + listComponents + '}'
