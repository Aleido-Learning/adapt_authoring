...
MongooseDB.prototype.create = function(objectType, objectData, callback) {
  ...
  if (Model = this.getModel(objectType)) {
    ...
    instance.save(function (err, result, numAffected) {
      ...
      console.log('create');
      console.log('model', Model, 'objectData', objectData);
...
MongooseDB.prototype.update = function(objectType, conditions, updateData, callback) {
  ...
  if (Model = this.getModel(objectType)) {
    Model.findOne(conditions, function (err, doc) {
      ...
      // function for differences in strings
      function diffStrings(a, b) {
        ...
        for (let i = 0; i < maxLength; i++) {
          ...
          if (charA === charB) {
            ...
            if (charA) result += `[-${charA}-]`; // deleted in B
            if (charB) result += `{+${charB}+}`; // added in B
          }
        }
      ...
      // function for differences in json objects/arrays
      function findDifferences(obj1, obj2, path = '') {
        let diffs = [];
      
        if (typeof obj1 !== typeof obj2) {
          diffs.push({ path, val1: obj1, val2: obj2 });
          return diffs;
        }
        ...
        keys.forEach(key => {
          ...
          diffs = diffs.concat(findDifferences(obj1[key], obj2[key], newPath));
        });
      
        return diffs;
      }

      // function for whether the same in json objects/arrays
      function isEqual(a, b) {
        ...
        if (typeof a === 'object') {
          if (Array.isArray(a) !== Array.isArray(b)) {
            console.log('isArray: ', Array.isArray(a), Array.isArray(b));
            return false;
          }
      ...
      for (var field in updateData) {
        // console.log('MongooseDB.prototype.update', field, doc[field], updateData[field]);
        if(doc[field] !== null && updateData[field] !== null) {
          let fieldDiff = false;
          switch(typeof doc[field]) {
            case 'boolean':
            case 'string':
              if(updateData[field] !== doc[field]) fieldDiff = true;
              break;
            case 'object':
             /* if (Array.isArray(doc[field])) {//'array (JSON-like)'} else {//'object (JSON-like)'}*/
              if(!isEqual(updateData[field], doc[field])) fieldDiff = true;
              break;
          }
          if(fieldDiff === true) {
            // console.log('updated: ', field, doc[field].toString(), updateData[field].toString());
            console.log('update');
            if(typeof doc[field] === 'boolean') console.log('changes in boolean: ', field + ': ', doc[field], updateData[field]);
            if(typeof doc[field] === 'string') console.log('changes in string: ', field + ': ', diffStrings(doc[field], updateData[field]));
            if(typeof doc[field] === 'object') console.log('changes in json: ', field + ': ', findDifferences(doc[field], updateData[field]));
          }
        }
...
MongooseDB.prototype.destroy = function(objectType, conditions, callback) {
  ...
  if (Model = this.getModel(objectType)) {
    console.log('destroy');
    console.log('model', Model, 'objectType', objectType, 'conditions', conditions);
